...
public class PlayerCollisions : MonoBehaviour {
	...
	void OnCollisionEnter2D (Collision2D col)
	{
		...
		if(col.gameObject.tag == "EnemyBullets")
		{
			...
			if(PlayerPrefs.GetInt("ShieldUpgradeHardened",0) == 1 && (bullet.name == "DogFighterBullet(Clone)" || bullet.name == "CruiserBullet(Clone)"))
			{
				//Subtract the health based on that bullet divided by 2
				takeDamage (bullet.getBulletDamage()/2);
			}
			else {
